...
import joblib
# These imports are required for joblib to deserialize the saved model/scaler/encoders,
# even though they are not referenced directly in the code below.
...
def calculate_age(df, date_column, new_column_name):
    ...
    for i in range(len(df)):
        ...
        if isinstance(date_value, str):          # parse string to date
    ...
    df[new_column_name] = pd.Series(ages_in_months, index=df.index)
...
def predict():
    try:
        # Accept both a plain JSON object body and the legacy double-encoded format
        # (where the client sends json=json.dumps(row_dict), adding an extra encoding layer).
        raw = request.get_json()
        data = json.loads(raw) if isinstance(raw, str) else raw

        df = pd.DataFrame([data], index=[0], columns=data.keys())

        # combine DPD fields from the two credit bureaus (worst delinquency wins)
        ...
        df = df.drop(columns=['data.Request.Input.CB1.MaxDPD', 'data.Request.Input.CB2.MaxDPD',
                               'data.Request.Input.CB1.CurrentDPD', 'data.Request.Input.CB2.CurrentDPD'])

        # sum outstanding loans across bureaus (a borrower may have debt at both)
        df['data.Request.Input.CB.Outstandingloan'] = (
            df[['data.Request.Input.CB1.Outstandingloan', 'data.Request.Input.CB2.Outstandingloan']]
        ...
        feature_df = df[loaded_features_before_selection].copy()

        # fill numerical NaN before encoding; categorical NaN handled post-encoding
        feature_df = feature_df.fillna(loaded_numerical_means)

        # target encode categorical features
        te = loaded_target_encoders_list
        for feature_name in feature_df.columns:
            if feature_name in te:
                feature_df[feature_name] = te[feature_name].transform(feature_df[feature_name])

        # fill any remaining NaN in now-encoded categorical columns
        feature_df = feature_df.fillna(loaded_categorical_means)

        features_scaled = pd.DataFrame(
            loaded_scaler.transform(feature_df),
            columns=feature_df.columns,
            index=feature_df.index
        )

        # restore binary features (scaling distorts 0/1 values)
        for col in ['salaryService', 'previousApplication',
                    'data.Request.Input.Customer.ExistingCustomer',
                    'data.Request.Input.Customer.Gender']:
            features_scaled[col] = feature_df[col]

        selected_feature_df = features_scaled[loaded_selected_features]
